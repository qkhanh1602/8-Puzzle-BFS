...
def bfs(start, goal):
    ...
    while frontier:
        # Lấy node đầu tiên ra khỏi frontier theo FIFO
        ...
        for next_state, action in get_neighbors(current_state):
            ...
            if next_key not in reached:
                ...
                child_info["status"] = "ADD vào frontier"
            else:
                child_info["status"] = "BỎ QUA vì đã có trong reached"
...
class EightPuzzleApp(ctk.CTk):
    ...
    def render_empty_trace(self):
        ...
        placeholder(self.node_card, "Bấm Solve\nđể xem BFS", "#94a3b8")
    ...
    def show_trace_at(self, index):
        ...
        if item["is_goal"]:
            ...
            for idx, child in enumerate(item["children"]):
                ...
                color = "#22c55e" if status_add else "#ef4444"
                status_text = "ADD" if status_add else "SKIP"
    ...
    def solve_puzzle(self):
        ...
        worker = threading.Thread(
            ...
            args=(start_str, goal_str),
            daemon=True
        )
        worker.start()

    def solve_puzzle_worker(self, start_str, goal_str):
